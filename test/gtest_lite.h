...
struct _Is_Types {
    ...
    template<typename D> static char(&f(...))[2];
    static bool const convertable = sizeof(f<ComponentType>(F())) == 1;
};
...
namespace gtest_lite {
    ...
    bool almostEQ(ComponentType a, ComponentType  b) {
        // eps: ha a relatív, vagy abszolút hiba ettől kisebb, akkor elfogadjuk
        ComponentType eps = 10 * std::numeric_limits<ComponentType>::epsilon(); // 10-szer a legkisebb érték
        if (a == b) return true;
        if (fabs(a - b) < eps)
            return true;
        double aa = fabs(a);
