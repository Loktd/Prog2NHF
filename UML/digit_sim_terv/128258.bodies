...
!!!133762.cpp!!!	InPin_Component(in inCount : size_t)
    inPinCount = inCount;
    activeInPins = 0;
...
!!!134018.cpp!!!	setInNodeID(in at : size_t, in id : size_t) : void
    if (at >= inPinCount) {
        throw std::string("Indexed out of inNodeID range...\n");
    }
    inNodeIDs[at] = id;
!!!134146.cpp!!!	connectedToNodeIn(in id : size_t) : bool
    for (size_t i = 0; i < inPinCount; i++) {
        if (inNodeIDs[i] == id) {
            return true;
        }
    ...
    return false;
!!!134274.cpp!!!	printInConnectedNodes(inout os : std::ostream) : void
    for (size_t i = 0; i < inPinCount; i++) {
        os << inNodeIDs[i];
        if (i != inPinCount - 1)
            os << " ";
    }
!!!134402.cpp!!!	resetForSimulation() : void
    activeInPins = 0;
!!!134530.cpp!!!	tickCounter() : void
    activeInPins++;
